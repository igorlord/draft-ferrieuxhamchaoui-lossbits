...
Since  explicit  path  signals  are preferred  by  {{!RFC8558}},  this
...
in this direction with a L bit equal to 1, as described in {{usage}}.
...
calculated in {{endtoendloss}}, then either  the Q Period is too short
for  the  amount of  packet  reordering  or  there is  observer  loss,
described in  {{observerloss}}. If  this happens, the  observer SHOULD
...
loss rate as  described in {{losscorrelation}} is  a strong indication
...
protocol extensions  per {{?RFC7285}}.  While an  explicit loss signal
-- a preferred way to share loss information per {{!RFC8558}} -- helps
